...
class RNG
{
 ...
 public:
  RNG() { init(); zigset(); }
  RNG(ulong x_) :
   z(x_), w(x_), jsr(x_), jcong(x_) {//printf(" zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz");
    zigset(); }
   RNG(ulong z_, ulong w_, ulong jsr_, ulong jcong_ ) :
   z(z_), w(w_), jsr(jsr_), jcong(jcong_) {//printf(" iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii");
     zigset(); }
  ~RNG() { }
...
#warning "Compiling RNG class for 64-bit architecture"
  ...
  ulong MWC()
  { ulong me =  (znew() << 16) + wnew();
    return ((me) & 0xfffffffful); }
  ulong SHR3()
    {
  ...
  ulong rand_int32()         // [0,2^32-1]
    ...
    ulong cong = CONG();
    ulong shr3 = SHR3();
    //printf(" here is %%%%%%%%%%%%%%%%%%%%% MWC %%%%%%%%%%%%%%%%%%%%% = %lu AND %%%%%%%%%%%%%%%%%%%%% CONG %%%%%%%%%%%%%%%%%%%%% = %lu AND %%%%%%%%%%%%%%%%%%%%% SHR3 %%%%%%%%%%%%%%%%%%%%% = %lu", mwc, cong, shr3);
  ...
  double RNOR() {
   ...
   // printf(" here is = %ld AND i is = %ld",h,i);
  ...
  {
      // printf("Inside Uniform RNG.h call\n");
      // printf("counter = %d\n", countr);
      //countr++;
